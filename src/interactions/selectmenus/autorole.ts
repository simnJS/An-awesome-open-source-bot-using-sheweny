...
export class Autorole extends SelectMenu {
  ...
  async execute(selectMenu: SelectMenuInteraction) {
    ...
    selectMenu.values.forEach(v => {
      ...
      switch (v) {
        case "first_option":
          if ((member?.roles as GuildMemberRoleManager).cache.has(`${css?.id}`)) {
            if (css) {
        ...
        case "second_option":
          if ((member?.roles as GuildMemberRoleManager).cache.has(`${html?.id}`)) {
            if (html) {
        ...
        case "third_option":
          if ((member?.roles as GuildMemberRoleManager).cache.has(`${js?.id}`)) {
            if (js) {
        ...
        case "fourth_option":
          if ((member?.roles as GuildMemberRoleManager).cache.has(`${Python?.id}`)) {
            if (Python) {
        ...
        case "fifth_option":
          if ((member?.roles as GuildMemberRoleManager).cache.has(`${batch?.id}`)) {
            if (batch) {
        ...
        case "sixth_option":
          if ((member?.roles as GuildMemberRoleManager).cache.has(`${java?.id}`)) {
            if (java) {
        ...
        case "seventh_option":
          if ((member?.roles as GuildMemberRoleManager).cache.has(`${php?.id}`)) {
            if (php) {
        ...
        case "eighth_option":
          if ((member?.roles as GuildMemberRoleManager).cache.has(`${lua?.id}`)) {
            if (lua) {
        ...
        case "ninth_option":
          if ((member?.roles as GuildMemberRoleManager).cache.has(`${csharp?.id}`)) {
            if (csharp) {
        ...
        case "tenth_option":
          if ((member?.roles as GuildMemberRoleManager).cache.has(`${c?.id}`)) {
            if (c) {
    ...
    if (added_roles.length > 0) {
      selectMenu.reply({ content: `Les roles ${added_roles.join(`, `)} vous ont été mis à jours !`, ephemeral: true })
